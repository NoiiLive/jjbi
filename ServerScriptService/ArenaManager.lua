...
local function ProcessTurn(match)
	...
	for _, attacker in ipairs(allCombatants) do
		...
		local dummyOpponent = {
			TotalStrength = highestStr,
			Traits = {}
		}
		for i=1, domCount do table.insert(dummyOpponent.Traits, "Dominating") end
		...
		local dummyRemote = {
			FireClient = function(_, plr, ev, data)
				table.insert(logMessages, data.LogMsg)
			end
		}

		local fz = CombatCore.ApplyStatusDamage(attacker, uniModStr, dummyRemote, nil, dummyBattle, 0)
		if fz == "Frozen" then
