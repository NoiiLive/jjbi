...
local function GenerateDungeonEnemy(template, dungeonId)
	...
	return {
		...
		Cooldowns = {}, 
		Stand = template.Stand or "None",
		Style = template.Style or "None",
		Skills = CombatCore.GetNPCSkills(template.Stand, template.Style),
...
local function StartDungeon(player, dungeonId)
	...
	CombatCore.ApplyPreCombatPassives(player, pData, firstEnemy)
...
DungeonAction.OnServerEvent:Connect(function(player, actionType, actionData)
	...
	elseif dungeon.Enemy.HP < 1 then
		...
		if dungeon.IsEndless then
			...
			CombatCore.ApplyPreCombatPassives(player, dungeon.Player, dungeon.Enemy)
