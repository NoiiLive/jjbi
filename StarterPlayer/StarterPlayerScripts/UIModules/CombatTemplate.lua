...
function CombatTemplate.Create(parentGui, tooltipMgr)
	...
	local function updateAbilitiesGrid()
		...
		local maxCellH = math.floor((abilitiesArea.AbsoluteSize.Y - totalPaddingY) / rows) - 10 

		cellW = math.min(cellW, 180)
		local targetH = isPortrait and 45 or (isMedium and 40 or 55)
		local cellH = math.min(maxCellH, targetH)
	...
	local function formatGrid(layout, container, count, isPortrait)
		...
		for _, f in pairs(container:GetChildren()) do
			if f:IsA("Frame") and f:FindFirstChild("IconBox") and f:FindFirstChild("InfoArea") then
				local desiredIconSize = math.min(h - 4, w * 0.3)
				f.IconBox.Size = UDim2.new(0, desiredIconSize, 0, desiredIconSize)
				f.InfoArea.Size = UDim2.new(1, -(desiredIconSize + 6), 1, 0)
			end
		end
	end
	...
	function combatUI:AddFighter(isAlly, id, name, iconId, initialHp, maxHp)
		...
		hpContainer.Size = UDim2.new(0.95, 0, 0.25, 0)
		...
		statusContainer.Size = UDim2.new(1, 0, 0.40, 0)
		...
		function fighterObj:SetStatus(statusId, iconString, durationText, descText)
			...
			if not existing then
				...
				existing.Size = UDim2.new(0, 20, 1, 0)
		...
		function fighterObj:SetCooldown(cdId, iconString, durationText, descText)
			self:SetStatus(cdId, iconString, durationText, descText)
		end
